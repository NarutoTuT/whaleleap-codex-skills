...
Run the sync script:
```bash
node "${CODEX_SKILLS_DIR:-$HOME/.agents/skills}/brand/scripts/sync-brand-to-tokens.cjs"
```
...
node "${CODEX_SKILLS_DIR:-$HOME/.agents/skills}/brand/scripts/inject-brand-context.cjs" --json | head -30
